"""Ensemble Kalman Filter (EnKF) for causal state estimation.

Key parameter: --obs-every-k controls how often observations are incorporated.
  K=1:   update every 1ms (filtering, not comparable to other models)
  K=20:  update every 20ms (50 Hz, realistic BCI control rate)
  K=100: update every 100ms (10 Hz, conservative)
  K=200: update never within window (same as encoder-based models)

Uses the acausal LatentNeuralODE's learned ODE dynamics as the dynamics model
and its linear decoder as the observation model. No additional training needed.
...
Evaluation uses 200-step windowed R² for comparison with other models.
...
# --------------------------------------------------------------------------- #
#  Batched Ensemble Kalman Filter
# --------------------------------------------------------------------------- #

class BatchedEnKF:
    """EnKF that processes W windows × N particles simultaneously.

    All windows of a trial are batched into a single tensor for GPU efficiency.

    Parameters
    ----------
    ode_func_net : nn.Sequential
        MLP mapping [z, u] -> dz/dt
    decoder : nn.Linear
        Maps z -> x_hat
    z_dim, n_x : int
    n_ensemble : int
    ...
    @torch.no_grad()
    def run_batched(self, z0_batch, u_batch, x_obs_batch, dt, obs_every_k=1):
        ...
        obs_every_k : int
            Only incorporate observations every K timesteps.
            K=1: update every step (filtering). K=200: update never (blind prediction).
            Between updates, predictions come from ODE only (no Kalman correction).
        ...
        # Precompute observation noise identity
        ...
        for t in range(T):
            ...
            # ---- UPDATE (only every obs_every_k steps) ----
            do_update = (t % obs_every_k == 0)
            ...
            # Record prediction from posterior (or prior if no update) mean
            z_mean = z_ens.mean(dim=1)  # (W, z_dim)
...
def evaluate_enkf(
    ...
    window_batch_size=64, obs_every_k=1,
):
    ...
    return {
        "Q": Q, "R": R, "n_ensemble": n_ensemble, "obs_every_k": obs_every_k,
...
def main():
    parser = argparse.ArgumentParser(description="EnKF evaluation using acausal LatentNeuralODE")
    ...
    parser.add_argument("--output-dir", type=str, default="results/enkf")
    parser.add_argument("--device", type=str, default="auto")
    parser.add_argument("--n-ensemble", type=int, default=64)
    parser.add_argument("--horizon", type=int, default=200)
    parser.add_argument("--stride", type=int, default=200)
    parser.add_argument("--obs-every-k", type=int, default=1,
                        help="Incorporate observations every K timesteps. K=1=every step (filtering), K=200=blind prediction.")
    ...
    print(f"\n{'='*70}")
    print(f"EnKF Sweep: {len(Q_values)} Q × {len(R_values)} R = {len(Q_values)*len(R_values)} configs")
    ...
    print(f"  Observation update rate: every {args.obs_every_k}ms ({1000/args.obs_every_k:.0f} Hz)")
    print(f"{'='*70}")
    ...
    print(f"{'='*70}")
    print(f"{'Q':>8s} {'R':>8s} {'R²':>10s} {'MSE':>12s} {'Time(ms)':>10s}")
    print("-" * 52)
    ...
    for N_ens in [32, 64, 128]:
        ...
        print(f"  N={N_ens:4d}: R²={r['avg_r2']:.4f}, MSE={r['avg_mse']:.6f}, "
    ...
    print(f"{'='*70}")
    print(f"  Best acausal (LatentNODE, full-seq R²=0.9387): bidirectional encoder (acausal)")
    print(f"  Best causal  (LatentCANODE, R²=0.8252):        causal encoder + CA-ODE")
    print(f"  EnKF (this run):                               R²={best_config['avg_r2']:.4f}")
