#!/usr/bin/env python3
"""Train Latent CA-NODE on spike-sorted neural data.

Pure reconstruction (no distillation) from sorted spike trains.
Handles variable neuron counts via masking of zero-padded channels.
...
Data lives in placement groups within an HDF5 file:
    ...
    placement_<p>/u:                   (n_trials, 2, n_bins*10) -- 1ms resolution

Usage:
    python -m modeling.scripts.train_spiking_canode \
        --data data/spiking_plant3.h5 --placement 0 \
        --z-dim 64 --hidden 256 --lr 5e-4 --epochs 200 \
        --output-dir results/spiking_canode_p0/z64_h256_lr5e-4 \
        --device cuda
"""
...
def load_spiking_data(
    ...
    """Load spike-sorted data from a placement group.

    Returns
    -------
    windows_x : ndarray (N_windows, W, max_n_sorted)  -- firing rates in Hz
    windows_u : ndarray (N_windows, W, 2)
    masks      : ndarray (N_windows, max_n_sorted)  -- 1 for real neurons, 0 for padding
    x_test_n, u_test_n, masks_test : test trial arrays (normalized)
    stats : dict of normalization parameters
    max_n_sorted : int
    """
    ...
    x_rates = x_sorted.astype(np.float32) / BIN_WIDTH_S  # counts / bin_width -> Hz

    # Subsample u from 1ms to 10ms (average within each bin)
    u_10ms = u_1ms.reshape(n_trials, n_u, n_bins, 10).mean(axis=-1)  # (n_trials, 2, n_bins)

    # Build per-trial masks: 1 for real neurons, 0 for padding
    ...
    x_train = x_rates[train_idx]    # (n_train, max_n, n_bins)
    u_train = u_10ms[train_idx]     # (n_train, 2, n_bins)
    m_train = neuron_masks[train_idx]
    x_test = x_rates[test_idx]
    ...
    # For u, straightforward (no masking needed)
    u_mean = u_train.mean(axis=(0, 2))  # (2,)
    u_std = u_train.std(axis=(0, 2)) + 1e-8

    # Normalize
    x_train_n = (x_train - x_mean[None, :, None]) / x_std[None, :, None]
    u_train_n = (u_train - u_mean[None, :, None]) / u_std[None, :, None]
    x_test_n = (x_test - x_mean[None, :, None]) / x_std[None, :, None]
    u_test_n = (u_test - u_mean[None, :, None]) / u_std[None, :, None]

    # Zero out padded neurons in normalized data too
    ...
    for trial in range(x_train_n.shape[0]):
        for t in range(0, n_bins - W, past_steps):  # stride = past_steps
            win_x.append(x_train_n[trial, :, t:t+W].T)  # (W, max_n)
    ...
    return windows_x, windows_u, masks, x_test_n, u_test_n, m_test, stats, max_n
...
def masked_mse(pred, target, mask):
    """MSE loss only over non-padded neurons.

    Parameters
    ----------
    pred   : (B, T, n_x)
    target : (B, T, n_x)
    mask   : (B, n_x) -- 1 for real, 0 for padded
    """
    mask_expanded = mask.unsqueeze(1)  # (B, 1, n_x) -- broadcast over T
    diff_sq = (pred - target) ** 2     # (B, T, n_x)
    ...
    return masked_diff.sum() / n_valid.clamp(min=1)
...
def main(args):
    ...
    dt_s = BIN_WIDTH_S  # 10ms per step

    print("=" * 60)
    print("Spiking CA-NODE Training")
    ...
    (windows_x, windows_u, masks,
     x_test_n, u_test_n, m_test, stats, max_n_sorted) = load_spiking_data(
    ...
    # -- Prepare datasets --------------------------------------------------
    all_x = torch.tensor(windows_x, dtype=torch.float32)  # (N, W, n_x)
    all_u = torch.tensor(windows_u, dtype=torch.float32)  # (N, W, n_u)
    all_m = torch.tensor(masks, dtype=torch.float32)       # (N, n_x)
    ...
    # Train/val split (90/10 of windows)
    ...
    # -- Time tensor for ODE integration -----------------------------------
    # Future window: fw steps at dt_s each
    ...
    # -- Training loop -----------------------------------------------------
    print("\nTraining...")
    history = {"train_loss": [], "val_loss": [], "lr": [], "val_loss_euler": []}
    best_val = float("inf")
    t0 = time.time()
    ...
    for epoch in range(args.epochs):
        ...
        for x_past, u_past, x_future, u_future, mask in train_loader:
            ...
            # Forward: causal encoder sees past, ODE integrates over future
            x_pred, mu, logvar = model.forward(
                x_past, u_past, u_future, t_future,
                method="dopri5", deterministic=True,
            )

            # Pure reconstruction loss with masking
            loss = masked_mse(x_pred, x_future, mask)
        ...
        with torch.no_grad():
            for x_past, u_past, x_future, u_future, mask in val_loader:
                ...
                val_loss_sum += masked_mse(x_pred, x_future, mask).item()

                # euler
                x_pred_e, _, _ = model.forward(
                    x_past, u_past, u_future, t_future,
                    method="euler", deterministic=True,
                )
                val_loss_euler_sum += masked_mse(x_pred_e, x_future, mask).item()
                val_n += 1
    ...
    for method in ["dopri5", "euler"]:
        ...
        with torch.no_grad():
            for trial in range(x_test_n.shape[0]):
                x_trial = torch.tensor(x_test_n[trial].T, dtype=torch.float32,
                                       device=device)  # (n_bins, max_n)
                u_trial = torch.tensor(u_test_n[trial].T, dtype=torch.float32,
                                       device=device)  # (n_bins, n_u)
                ...
                for t in range(0, n_bins - pw - fw, pw):
                    ...
                    x_pred = model.predict(xp, up, uf, t_future, method=method)
                    x_pred = x_pred.squeeze(0)

                    all_preds.append(x_pred.cpu().numpy())
                    all_trues.append(xf.cpu().numpy())
        ...
        print("  %s: R2=%.4f, MSE=%.6f" % (method, r2, mse))
    ...
    plt.suptitle("Spiking CA-NODE: z=%d h=%d lr=%.0e p=%d" %
                 (args.z_dim, args.hidden, args.lr, args.placement))
    plt.tight_layout()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Latent CA-NODE on spike-sorted data")
    parser.add_argument("--data", type=str, required=True,
                        help="Path to spiking HDF5 file")
    parser.add_argument("--placement", type=int, default=0,
                        help="Placement index (default: 0)")
    ...
    # Windows (in 10ms steps)
    parser.add_argument("--past-window", type=int, default=20,
                        help="Past window in 10ms steps (default: 20 = 200ms)")
    parser.add_argument("--future-window", type=int, default=20,
                        help="Future window in 10ms steps (default: 20 = 200ms)")
