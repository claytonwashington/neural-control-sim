...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x_past, u_past, x_future, u_future, mask in train_loader:
            ...
            loss = masked_poisson_nll(log_rates_pred, x_future, mask)
        ...
        with torch.no_grad():
            for x_past, u_past, x_future, u_future, mask in val_loader:
                ...
                val_loss_sum += masked_poisson_nll(log_rates, x_future, mask).item()

                log_rates_e, _, _ = model.forward(
                    x_past, u_past, u_future, t_future,
                    method="euler", deterministic=True,
                )
                val_loss_euler_sum += masked_poisson_nll(log_rates_e, x_future, mask).item()
                val_n += 1
        ...
        with torch.no_grad():
            for trial in range(x_test.shape[0]):
                ...
                for t in range(0, n_bins - pw - fw, pw):
                    ...
                    log_rates = model.predict(xp, up, uf, t_future, method=method)
