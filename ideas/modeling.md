...
4. **Validation plots** *(auto-generated by `preflight complete`)* — true-vs-inferred
   firing rates across **all channels** (true/predicted/error heatmaps + top-variance
   traces) and projected onto the **top principal components**, plus `val_metrics.json`
   (overall / per-channel / per-PC R²). Surfaced on the experiment's
   `results/experiments/<slug>.html` page, linked from the leaderboard. Requires a
   saved `model.pt` and the correct `--model-type` at `preflight start`; otherwise the
   page is metadata-only.
5. **For control experiments** — tracking RMSE, settling time, steady-state error, control effort
6. **For top performers** — N4SID vs real data vs model prediction comparison figure
