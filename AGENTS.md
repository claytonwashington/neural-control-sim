...
4. **Per-experiment validation pages**: Each leaderboard model links (cyan ↗) to a standalone, self-contained page at `results/experiments/<slug>.html`.
   ...
   - **Control experiments** (closed-loop, e.g. optoclamp, `model_type: other`) show a **control view** instead: tracking plots (controlled rate vs. target) + a metrics table (tracking RMSE / settling / SS-error / overshoot, per controller × target) from the run's `optoclamp_results.json`. See `generate_experiment_pages._control_view`.
