...
4. **Per-experiment validation pages**: Each leaderboard model links (cyan ↗) to a standalone, self-contained page at `results/experiments/<slug>.html`.
   - **Digital-twin models** show **true-vs-inferred firing rates across all channels (heatmaps + top-variance traces) and projected onto the top principal components**, plus `val_metrics.json` (overall / per-channel / per-PC R²). Validation inference is implemented for `model_type ∈ {canode, latent_canode, latent_node, gru, n4sid}` (`SUPPORTED_MODEL_TYPES` in `dashboard_common.py`; N4SID is a raw-space linear state-space `.npz`, the rest are torch `model.pt`). To add a new model family: add a branch to `validation_plots.load_model`/`infer` and the type to `SUPPORTED_MODEL_TYPES`.
   - **Control experiments** (closed-loop, e.g. optoclamp, `model_type: other`) show a **control view** instead: tracking plots (controlled rate vs. target), control-signal traces, and a metrics table (tracking RMSE / settling / SS-error / overshoot, per controller × target) from the run's `optoclamp_results.json`. See `generate_experiment_pages._control_view`. **`run_optoclamp.py` persists per-step trajectories** (rate + control signal per controller × target) to `optoclamp_trajectories.npz` so the figures can be regenerated without re-running the (slow) closed-loop sim: `python -m modeling.scripts.run_optoclamp --replot-from <…>/optoclamp_trajectories.npz`. (Runs before this addition saved only summary metrics, so their pages can't show full trajectories until re-run.)
   - Pages with no plots state **why** (no checkpoint / unsupported type / not yet generated — `dashboard_common.no_plots_reason`). The headline acausal Latent NODE was recovered (its sweep never saved `model.pt`) into `results/latent_node_recovered/`.
...
9. **Always persist checkpoints**: every sweep run MUST save its model to `<run_dir>/model.pt` (the `sweep_*.py` scripts pass `--save-model <output_dir>/<run_id>/model.pt`). This is what lets the validation pipeline (`find_checkpoint` / `manifest_to_entry`) load the model later. The acausal Latent NODE plots were lost for months because an early sweep saved only a training curve — don't repeat that.
